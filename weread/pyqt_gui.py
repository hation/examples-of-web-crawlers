...
def get_books(books):
    return [[book.bookId, book.title, book.author, book.translator, book.cover, book.type, book.price, book.total_words, book.chapter_size, book.publish_time, book.copyright_info_name, book.rating_title, book.rating_count
                          , book.rating_good, book.rating_fair, book.rating_poor, book.rating_deep_v, book.intro, book.ai_summary, book.cate] for book in books]
...
if __name__=='__main__':
    ...
    books_finish_read = get_books(books['finishReadBooks'])
    books_recent_read = books['recentBooks']
    books_recent_read = get_books(books['recentBooks'])
    books_all = books['allBooks']
    books_all = [[book.bookId, book.title, book.author, book.translator, book.cover, book.type, book.price, book.total_words, book.chapter_size, book.publish_time, book.copyright_info_name, book.rating_title, book.rating_count
                          , book.rating_good, book.rating_fair, book.rating_poor, book.rating_deep_v, book.intro, book.ai_summary, book.cate] for book in books_all]
    write_excel_xls(data_dir + '我的书架.xls', ['已读完的书籍', '最近阅读的书籍', '所有的书籍'], [["ID", "书名", "作者", "译者", "封面","分类","价格", "总字数", "总章节数","出版时间",
                                                                              "出版方", "读书推荐值", "总评分数", "推荐数", "一般数", "不行数", "资深会员数", "介绍", "AI摘要", "目录"], ]) # 写入excel文件
